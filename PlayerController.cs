...
public class PlayerController : MonoBehaviour
{
    ...
    //Character_Controller component of the player
    ...
    private void SetUpCharacterController() 
    {
        ...
        if (controller == null) 
        {
            Debug.LogError("The player controller script does not have a character controller on the same object");
        }
    }
    ...
    {
        //get the imput from the input manager
        ...
        if(controller.isGrounded)
        {
            ...
            //Set the movement direction to be received input , set y to 0 since we are on ground 
        ...
        else 
        {
            ...
            moveDirection = transform.TransformDirection(moveDirection);
            //now our player will be able to move while in the air
        ...
        if (controller.isGrounded && moveDirection.y < 0) 
        {
            ...
            //this prevents gravity from stacking up on the player while they are grounded
            //so that whem a player walks up a ledge they do not move at an unreasonable falling speed
    ...
    {
        float horizonatlLookInput = inputManager.horizontalLookAxis;
        Vector3 playerRotation = transform.rotation.eulerAngles;
        transform.rotation = Quaternion.Euler(new Vector3(playerRotation.x, playerRotation.y + horizonatlLookInput * lookSpeed *
